...
def handle_combined_input(squat_controller, walk_controller):
    ...
    cmd_vel[0] = 0.5 if key_states['w'] else (-0.5 if key_states['s'] else 0.0)  # Forward/backward
    cmd_vel[1] = 0.5 if key_states['a'] else (-0.5 if key_states['d'] else 0.0)  # Yaw left/right    
    cmd_vel[2] = 0.5 if key_states['q'] else (-0.5 if key_states['e'] else 0.0)  # angular left/right

    # Check for height commands (squatting)
    height_cmd = squat_controller.height_cmd
    if key_states['r']:
        height_cmd = min(height_cmd + 0.0008, 1.04)
    if key_states['f']:
        height_cmd = max(height_cmd - 0.0008, 0.65)
